...
__attribute__((noreturn)) void yield_forever()
{
  while (1)
  {
    yield();
...
__attribute__((noreturn)) void run_forever()
{
  while (1)
  {
    __asm__("");
...
__attribute__((noreturn)) void iowait_forever()
{
  int fds[2];
  pipe(fds);
  while (1)
...
__attribute__((noreturn)) void exit_fast()
{
  exit();
}
...
void wait_for_ticket_counts(int num_children, int *pids, int *tickets)
{
  ...
  set_tickets(NOT_AS_LARGE_TICKET_COUNT);
  for (int yield_count = 0; yield_count < MAX_YIELDS_FOR_SETUP; ++yield_count)
  {
    ...
    for (int i = 0; i < num_children; ++i)
    {
      ...
      if (info.tickets[index] != tickets[i])
        done = 0;
    }
    if (done)
      break;
  }
  set_tickets(LARGE_TICKET_COUNT);
}
...
int compare_ticks_chi_squared(struct test_case *test)
{
  ...
  */
  ...
  for (int i = 0; i < test->num_children; ++i)
  {
    long long scaled_expected = (test->expect_ticks_unscaled[i] << FIXED_POINT_COUNT) / expect_ticks_total * test->total_actual_ticks;
#ifdef DEBUG
